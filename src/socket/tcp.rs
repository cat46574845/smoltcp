...
/// Use `Socket<'a, LinearBuffer<'a>>` for non-wrapping linear buffers.
...
/// # Type Parameters
///
/// - `B`: The buffer type, must implement `SocketBufferT`. Defaults to `SocketBuffer`
///   (which is `RingBuffer<u8>`). Use `LinearBuffer` for non-wrapping behavior.
#[derive(Debug)]
pub struct Socket<'a, B: SocketBufferT<'a> = SocketBuffer<'a>> {
    state: State,
    timer: Timer,
    rtte: RttEstimator,
    assembler: Assembler,
...
impl<'a, B: SocketBufferT<'a>> fmt::Write for Socket<'a, B> {
...
mod test {
    ...
    struct TestSocket<B: SocketBufferT<'static> = SocketBuffer<'static>> {
        socket: Socket<'static, B>,
        cx: Context,
    }

    impl<B: SocketBufferT<'static>> Deref for TestSocket<B> {
        type Target = Socket<'static, B>;
        fn deref(&self) -> &Self::Target {
            &self.socket
        }
    }

    impl<B: SocketBufferT<'static>> DerefMut for TestSocket<B> {
    ...
    #[track_caller]
    fn send<B: SocketBufferT<'static>>(
        socket: &mut TestSocket<B>,
        timestamp: Instant,
        repr: &TcpRepr,
    ...
    #[track_caller]
    fn recv<B: SocketBufferT<'static>, F>(socket: &mut TestSocket<B>, timestamp: Instant, mut f: F)
    where
    ...
    #[track_caller]
    fn recv_nothing<B: SocketBufferT<'static>>(socket: &mut TestSocket<B>, timestamp: Instant) {
        socket.cx.set_now(timestamp);
    ...
    // Trait for creating test buffers of different types
    trait TestBufferFactory<'a>: SocketBufferT<'a> {
        fn create(size: usize) -> Self;
    }

    impl<'a> TestBufferFactory<'a> for SocketBuffer<'a> {
        fn create(size: usize) -> Self {
            SocketBuffer::new(vec![0u8; size])
        }
    }

    impl<'a> TestBufferFactory<'a> for crate::storage::LinearBuffer<'a> {
        fn create(size: usize) -> Self {
            crate::storage::LinearBuffer::new(vec![0u8; size])
        }
    }

    fn socket_generic<B: TestBufferFactory<'static>>(tx_len: usize, rx_len: usize) -> TestSocket<B> {
        let (iface, _, _) = crate::tests::setup(crate::phy::Medium::Ip);

        let rx_buffer = B::create(rx_len);
        let tx_buffer = B::create(tx_len);
    ...
    fn socket_syn_received_generic<B: TestBufferFactory<'static>>(tx_len: usize, rx_len: usize) -> TestSocket<B> {
        let mut s = socket_generic::<B>(tx_len, rx_len);
    ...
    fn socket_syn_sent_generic<B: TestBufferFactory<'static>>(tx_len: usize, rx_len: usize) -> TestSocket<B> {
        let mut s = socket_generic::<B>(tx_len, rx_len);
    ...
    fn socket_established_generic<B: TestBufferFactory<'static>>(tx_len: usize, rx_len: usize) -> TestSocket<B> {
        let mut s = socket_syn_received_generic::<B>(tx_len, rx_len);
    ...
    fn socket_fin_wait_1_generic<B: TestBufferFactory<'static>>() -> TestSocket<B> {
        let mut s = socket_established_generic::<B>(64, 64);
        s.state = State::FinWait1;
        s
    }

    fn socket_fin_wait_2_generic<B: TestBufferFactory<'static>>() -> TestSocket<B> {
        let mut s = socket_fin_wait_1_generic::<B>();
    ...
    fn socket_closing_generic<B: TestBufferFactory<'static>>() -> TestSocket<B> {
        let mut s = socket_fin_wait_1_generic::<B>();
    ...
    fn socket_time_wait_generic<B: TestBufferFactory<'static>>(from_closing: bool) -> TestSocket<B> {
        let mut s = socket_fin_wait_2_generic::<B>();
    ...
    fn socket_close_wait_generic<B: TestBufferFactory<'static>>() -> TestSocket<B> {
        let mut s = socket_established_generic::<B>(64, 64);
    ...
    fn socket_last_ack_generic<B: TestBufferFactory<'static>>() -> TestSocket<B> {
        let mut s = socket_close_wait_generic::<B>();
        s.state = State::LastAck;
        s
    }

    // Non-generic wrappers for backwards compatibility (use RingBuffer)
    fn socket() -> TestSocket {
        socket_with_buffer_sizes(64, 64)
    }

    fn socket_with_buffer_sizes(tx_len: usize, rx_len: usize) -> TestSocket {
        socket_generic::<SocketBuffer<'static>>(tx_len, rx_len)
    }

    fn socket_syn_received_with_buffer_sizes(tx_len: usize, rx_len: usize) -> TestSocket {
        socket_syn_received_generic::<SocketBuffer<'static>>(tx_len, rx_len)
    }
    ...
    fn socket_syn_sent_with_buffer_sizes(tx_len: usize, rx_len: usize) -> TestSocket {
        socket_syn_sent_generic::<SocketBuffer<'static>>(tx_len, rx_len)
    }
    ...
    fn socket_established_with_buffer_sizes(tx_len: usize, rx_len: usize) -> TestSocket {
        socket_established_generic::<SocketBuffer<'static>>(tx_len, rx_len)
    }
    ...
    fn socket_fin_wait_1() -> TestSocket {
        socket_fin_wait_1_generic::<SocketBuffer<'static>>()
    }

    fn socket_fin_wait_2() -> TestSocket {
        socket_fin_wait_2_generic::<SocketBuffer<'static>>()
    }

    fn socket_closing() -> TestSocket {
        socket_closing_generic::<SocketBuffer<'static>>()
    }

    fn socket_time_wait(from_closing: bool) -> TestSocket {
        socket_time_wait_generic::<SocketBuffer<'static>>(from_closing)
    }

    fn socket_close_wait() -> TestSocket {
        socket_close_wait_generic::<SocketBuffer<'static>>()
    }

    fn socket_last_ack() -> TestSocket {
        socket_last_ack_generic::<SocketBuffer<'static>>()
    }
    ...
    // =========================================================================================//
    // LinearBuffer Tests - Generate same tests using LinearBuffer
    // =========================================================================================//
    // 
    // We use local scope shadowing to redefine socket factory functions for LinearBuffer.
    // This allows all tests to be reused with the new buffer type.

    mod linear_buffer_full_tests {
        use super::*;
        use crate::storage::LinearBuffer;

        // Type alias for the buffer type used in tests
        type BufferType<'a> = LinearBuffer<'a>;

        // Shadow socket factory functions to use LinearBuffer
        fn socket() -> TestSocket<LinearBuffer<'static>> {
            socket_with_buffer_sizes(64, 64)
        }

        fn socket_with_buffer_sizes(tx_len: usize, rx_len: usize) -> TestSocket<LinearBuffer<'static>> {
            socket_generic::<LinearBuffer<'static>>(tx_len, rx_len)
        }

        fn socket_syn_received_with_buffer_sizes(tx_len: usize, rx_len: usize) -> TestSocket<LinearBuffer<'static>> {
            socket_syn_received_generic::<LinearBuffer<'static>>(tx_len, rx_len)
        }

        fn socket_syn_received() -> TestSocket<LinearBuffer<'static>> {
            socket_syn_received_with_buffer_sizes(64, 64)
        }

        fn socket_syn_sent_with_buffer_sizes(tx_len: usize, rx_len: usize) -> TestSocket<LinearBuffer<'static>> {
            socket_syn_sent_generic::<LinearBuffer<'static>>(tx_len, rx_len)
        }

        fn socket_syn_sent() -> TestSocket<LinearBuffer<'static>> {
            socket_syn_sent_with_buffer_sizes(64, 64)
        }

        fn socket_established_with_buffer_sizes(tx_len: usize, rx_len: usize) -> TestSocket<LinearBuffer<'static>> {
            socket_established_generic::<LinearBuffer<'static>>(tx_len, rx_len)
        }

        fn socket_established() -> TestSocket<LinearBuffer<'static>> {
            socket_established_with_buffer_sizes(64, 64)
        }

        fn socket_fin_wait_1() -> TestSocket<LinearBuffer<'static>> {
            socket_fin_wait_1_generic::<LinearBuffer<'static>>()
        }

        fn socket_fin_wait_2() -> TestSocket<LinearBuffer<'static>> {
            socket_fin_wait_2_generic::<LinearBuffer<'static>>()
        }

        fn socket_closing() -> TestSocket<LinearBuffer<'static>> {
            socket_closing_generic::<LinearBuffer<'static>>()
        }

        fn socket_time_wait(from_closing: bool) -> TestSocket<LinearBuffer<'static>> {
            socket_time_wait_generic::<LinearBuffer<'static>>(from_closing)
        }

        fn socket_close_wait() -> TestSocket<LinearBuffer<'static>> {
            socket_close_wait_generic::<LinearBuffer<'static>>()
        }

        fn socket_last_ack() -> TestSocket<LinearBuffer<'static>> {
            socket_last_ack_generic::<LinearBuffer<'static>>()
        }

        fn socket_recved() -> TestSocket<LinearBuffer<'static>> {
            let mut s = socket_established();
            send!(
                s,
                TcpRepr {
        ...
        fn socket_listen() -> TestSocket<LinearBuffer<'static>> {
            let mut s = socket();
            s.state = State::Listen;
            s.listen_endpoint = LISTEN_END;
            s
        ...
        // but for now we use the test generation macro approach.
        // 
        // Since Rust's macro system cannot easily duplicate function definitions from
        // the parent scope, we use a different approach: generate tests using the 
        // generic socket factory functions directly.

        include!("tcp_linear_buffer_tests.inc.rs");
    }
}
